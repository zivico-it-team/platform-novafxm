const axios = require('axios');cconst WebSocket = require('ws');
...
function startPriceStream(onPrices) {
  ...
  const connect = () => {
    ...
    socket.on('error', (error) => scannerErrors.set('twelveDataStream', error.message));
    socket.on('close', () => {
...
module.exports = { instruments, getPrices, getPrice, getCandles, getFeedStatus };
.open),
    high: Number(value.high),
    low: Number(value.low),
    close: Number(value.close),
