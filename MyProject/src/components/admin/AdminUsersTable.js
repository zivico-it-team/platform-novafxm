      ...
      <ScrollView horizontal>
        <View style={{ minWidth: 1900 }}>
          ...
          {users.map((user) => {
            ...
            const isDemoAccount = String(user.accountType || '').toLowerCase() === 'demo';
            const verificationOnly = isDemoAccount && user.verificationStatus !== 'approved';
            return (
              <View key={user.id} className="flex-row border-b border-border/60">
                ...
                <TextCell width={130} className={verificationOnly ? 'text-muted' : ''}>{verificationOnly ? '-' : `$${money(user.wallet?.balance)}`}</TextCell>
                <TextCell width={120} className={verificationOnly ? 'text-muted' : ''}>{verificationOnly ? '-' : `$${money(user.wallet?.equity)}`}</TextCell>
                <TextCell width={110} className={verificationOnly ? 'text-muted' : ''}>{verificationOnly ? '-' : `$${money(user.wallet?.margin)}`}</TextCell>
                <TextCell width={125} className={verificationOnly ? 'text-muted' : ''}>{verificationOnly ? '-' : `$${money(user.wallet?.freeFunds)}`}</TextCell>
                <TextCell width={90} className={verificationOnly ? 'text-muted' : ''}>{verificationOnly ? '-' : `1:${user.leverage || 100}`}</TextCell>
                <TextCell width={115} className={verificationOnly ? 'text-muted' : user.tradingStatus === 'frozen' ? 'text-danger' : 'text-success'}>{verificationOnly ? '-' : user.tradingStatus === 'frozen' ? 'Frozen' : 'Active'}</TextCell>
                ...
                <TextCell width={220} className="text-muted">{verificationOnly ? '-' : user.adminNotes || '-'}</TextCell>
                {verificationOnly ? (
