...
export default function TopAccountBar() {
  ...
  return (
    <View className={`${mobile ? 'relative z-40 gap-1.5 px-2 py-1.5' : 'relative z-40 border-b px-2 py-1.5'} lg:flex-row lg:items-center lg:gap-3 lg:px-3 lg:py-3`} style={{ backgroundColor: colors.background, borderColor: colors.border }}>
      ...
      {!mobile ? <Pressable onPress={() => setOrderModal(true)} className="mb-3 flex-row items-center justify-center rounded-xl px-5 py-4 lg:mb-0" style={{ backgroundColor: colors.primary }}><Plus color="#0B0B0B" size={18} /><Text className="ml-2 font-bold text-black">New Order</Text></Pressable> : null}
      ...
      {!mobile ? <Pressable onPress={() => setMenu(menu === 'account' ? null : 'account')} className="mt-3 flex-row items-center rounded-xl border px-4 py-3 lg:mt-0 lg:w-[250px]" style={{ backgroundColor: colors.panel, borderColor: colors.border }}><CircleUserRound color={colors.muted} size={23} /><View><Text className="ml-4 font-bold" style={{ color: colors.text }}>{selectedAccount?.type || 'Demo'} - {money(selectedAccountBalance)} USD</Text><Text className="ml-4 text-xs" style={{ color: colors.muted }}>{selectedAccount?.name || 'Demo account 1'}</Text></View><View className="ml-auto h-2.5 w-2.5 rounded-full" style={{ backgroundColor: colors.success }} /></Pressable> : null}
