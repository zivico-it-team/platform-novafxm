...
router.get('/trading-accounts', authMiddleware, async (req, res) => {
  try {
    ...
    const [accounts] = await connection.execute(
      `SELECT ta.id, ta.account_number, ta.account_type, ta.name, ta.balance, ta.equity,
  ...
  try {
    ...
    const [result] = await connection.execute(
      `INSERT INTO trading_accounts
        (user_id, account_number, account_type, name, balance, equity, free_margin, margin_level)
       VALUES (?, ?, ?, ?, ?, ?, ?, 0)`,
