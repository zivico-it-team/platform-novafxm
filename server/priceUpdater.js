...
const fetchYahooPrice = async (meta) => {
  if (!meta.yahoo) return null;

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 6000);

  try {
    const symbol = encodeURIComponent(meta.yahoo);
    ...
    if (!response.ok) {
      throw new Error(`Yahoo ${meta.yahoo} responded with ${response.status}`);
    }

    const payload = await response.json();
    const result = payload.chart?.result?.[0];
    const price = toNumber(result?.meta?.regularMarketPrice);

    return price == null ? null : [meta.symbol, price];
  } finally {
    clearTimeout(timeout);
  }
};
...
const fetchYahooPrices = async () => {
  const results = await Promise.allSettled(SYMBOL_META.map(fetchYahooPrice));
...
const fetchTradingViewPrices = async () => {
  ...
  try {
    ...
    console.warn(`Live price fetch failed for ${group}:`, error.message);
    return {};
  }
};
