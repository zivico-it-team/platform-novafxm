...
const INDEX_META = [
  ['ASX/AUD', 7800, 1, 0, 12],
  ['DAX/EUR', 18000, 1, 0, 20],
  ['DJI/USD', 38500, 1, 0, 30],
  ['ESX/EUR', 5000, 1, 0, 15],
  ['F40/EUR', 7600, 1, 0, 15],
  ['FTS/GBP', 7600, 1, 0, 15],
  ['HSI/HKD', 17000, 1, 0, 25],
  ['IBX/EUR', 11000, 1, 0, 15],
  ['NDX/USD', 18000, 1, 0, 25],
  ['NIK/JPY', 39000, 1, 0, 25],
  ['SPX/USD', 5200, 0.1, 1, 5],
];

const METAL_META = [
  ['XAG/AUD', 115, 0.001, 3, 30, 5000],
  ['XAG/CHF', 70, 0.001, 3, 30, 5000],
  ['XAG/EUR', 70, 0.001, 3, 30, 5000],
  ['XAG/GBP', 60, 0.001, 3, 30, 5000],
  ['XAG/USD', 76.5, 0.001, 3, 30, 5000],
  ['XAU/AUD', 7000, 0.01, 2, 40, 100],
  ['XAU/CHF', 4200, 0.01, 2, 40, 100],
  ['XAU/EUR', 4300, 0.01, 2, 40, 100],
  ['XAU/GBP', 3700, 0.01, 2, 40, 100],
  ['XAU/USD', 4685.6, 0.01, 2, 40, 100],
  ['XPD/USD', 1000, 0.01, 2, 60, 100],
  ['XPT/USD', 950, 0.01, 2, 50, 100],
];

const ENERGY_META = [
  ['BRN/USD', 85, 0.01, 2, 5, 1000],
  ['NGC/USD', 2.5, 0.001, 3, 8, 10000],
  ['WTI/USD', 80, 0.01, 2, 5, 1000],
];
...
const getListMeta = (category, [symbol, base, pip, digits, spread, contractSize = 1]) => ({
  symbol,
  category,
  base,
  pip,
...
const getSymbolMeta = (symbol) => SYMBOL_META.find((item) => item.symbol === symbol);

const calculateMarginRequired = ({ symbol, lotSize, openPrice, leverage }) => {
  const meta = getSymbolMeta(symbol);
  const contractSize = meta?.contractSize || 100000;
  return (Number(lotSize) * contractSize * Number(openPrice)) / Number(leverage || 1);
};

const calculatePnL = ({ symbol, type, lotSize, openPrice, closePrice }) => {
  const meta = getSymbolMeta(symbol);
  const contractSize = meta?.contractSize || 100000;
  const priceDiff = Number(closePrice) - Number(openPrice);
  const positionValue = priceDiff * Number(lotSize) * contractSize;

  return String(type).toLowerCase() === 'buy' ? positionValue : -positionValue;
};
