const { getCurrentPrices } = require('../priceUpdater');
...
const calculateAccountMetrics = (account, openTrades = []) => {
  ...
  const equity = roundMoney(balance + totals.floatingPnL);
  ...
  return { balance: roundMoney(balance), equity, used_margin: usedMargin, free_margin: freeMargin, margin_level: marginLevel };
};
