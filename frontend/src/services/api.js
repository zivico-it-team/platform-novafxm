...
export const accountAPI = {
  ...
  getBankInfos: async (token) => {
    try {
      const response = await fetch(`${API_URL}/account/bank-info`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      return await handleApiResponse(response);
    } catch (error) {
  ...
  createBankInfo: async (token, bankInfo) => {
    try {
      const response = await fetch(`${API_URL}/account/bank-info`, {
        method: 'POST',
        headers: {
...
export const pricesAPI = {
  ...
  getPrice: async (symbol) => {
    try {
      const response = await fetch(`${API_URL}/prices/${encodeURIComponent(symbol)}`);
      return await handleApiResponse(response);
    } catch (error) {
  ...
  getCandles: async (symbol, timeframe = '15', limit = 240) => {
    try {
      ...
      const response = await fetch(`${API_URL}/prices/candles?${params.toString()}`);
      return await handleApiResponse(response);
    } catch (error) {
...
export const adminAPI = {
  ...
  adjustLiveAccountBonus: async (token, accountId, amount, note = '') => {
    try {
      const response = await fetch(`${API_URL}/admin/live-accounts/${accountId}/bonus`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      console.error('Adjust live account bonus error:', error);
      return { error: error.message };
    }
  },
