...
export default function PositionsPanel({ trades, history, prices, onCloseTrade }) {
  const [activeTab, setActiveTab] = useState('positions');

  const formatPrice = (price, symbol) => {
    return formatSymbolPrice(symbol, price);
  };
                ...
                <tbody>
                  {trades.map((trade) => {
                    ...
                    const currentPrice = getNumericPrice(
                      trade.currentPrice ?? getCloseExecutionPrice(prices, trade),
                      meta?.base || 0
                    );
                    const pnl = getNumericPrice(trade.floatingPnL);
