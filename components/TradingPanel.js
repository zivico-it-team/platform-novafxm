...
import { getLiveQuote, getOpenExecutionPrice, getQuoteSpread } from '@/lib/marketQuotes';

export default function TradingPanel({ symbol, prices, account, onOpenTrade }) {
  ...
  const { bid: bidPrice, ask: askPrice, source } = getLiveQuote(prices, symbol);
  ...
  const canTrade = account && requiredMargin <= account.freeMargin;
  ...
  const handleOpenTrade = async (type) => {
    ...
    if (!canTrade) {
      setErrors({ form: 'Insufficient margin available' });
      return;
    }
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="p-4 border-b border-nova-border space-y-2 text-sm">
          <div className="flex justify-between">
            ...
            <span className="font-medium text-gray-900">{formatPrice(bidPrice)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">Ask</span>
            <span className="font-medium text-gray-900">{formatPrice(askPrice)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">Spread</span>
            <span className="font-medium text-gray-900">{getNumericPrice(liveSpread).toFixed(1)} pips</span>
          </div>
          <div className="flex justify-between">
        ...
        <div className="p-4 space-y-2 flex-1 flex flex-col justify-end">
          ...
          {!canTrade && (
            <p className="text-xs text-red-600 text-center mt-2">Insufficient margin</p>
          )}
          {errors.form && (
